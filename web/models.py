...
class PricePolicy(models.Model):
    ...
    category = models.SmallIntegerField(
        verbose_name="收费类型", default=2, choices=category_choices
    )
    ...
    project_space = models.PositiveIntegerField(
        verbose_name="单项目空间", help_text="G"
    )
    per_file_size = models.PositiveIntegerField(
        verbose_name="单文件大小", help_text="M"
    )
...
class Transaction(models.Model):
    ...
    order = models.CharField(
        verbose_name="订单号", max_length=64, unique=True
    )  # 唯一索引
...
class Project(models.Model):
    ...
    desc = models.CharField(
        verbose_name="项目描述", max_length=255, null=True, blank=True
    )

    use_space = models.BigIntegerField(
        verbose_name="项目已使用空间", default=0, help_text="字节"
    )
...
class FileRepository(models.Model):
    ...
    file_type = models.SmallIntegerField(
        verbose_name="类型", choices=file_type_choices
    )
    name = models.CharField(
        verbose_name="文件夹名称", max_length=32, help_text="文件/文件夹名"
    )
    key = models.CharField(
        verbose_name="文件储存在COS中的KEY", max_length=128, null=True, blank=True
    )

    # int类型最大表示的数据
    file_size = models.BigIntegerField(
        verbose_name="文件大小", null=True, blank=True, help_text="字节"
    )
...
class Issues(models.Model):
    ...
    issues_type = models.ForeignKey(
        verbose_name="问题类型", to="IssuesType", on_delete=models.CASCADE
    )
    ...
    attention = models.ManyToManyField(
        verbose_name="关注者", to="UserInfo", related_name="observe", blank=True
    )
    ...
    mode = models.SmallIntegerField(
        verbose_name="模式", choices=mode_choices, default=1
    )
    ...
    latest_update_datetime = models.DateTimeField(
        verbose_name="最后更新时间", auto_now=True
    )
...
class IssuesReply(models.Model):
    ...
    reply_type_choices = ((1, "修改记录"), (2, "回复"))
    reply_type = models.IntegerField(
        verbose_name="类型", choices=reply_type_choices
    )

    issues = models.ForeignKey(
        verbose_name="问题", to="Issues", on_delete=models.CASCADE
    )
...
class ProjectInvite(models.Model):
    ...
    count = models.PositiveIntegerField(
        verbose_name="限制数量", null=True, blank=True, help_text="空表示无数量限制"
    )
    ...
    period = models.IntegerField(
        verbose_name="有效期", choices=period_choices, default=1440
    )
    create_datetime = models.DateTimeField(
        verbose_name="创建时间", auto_now_add=True
    )
