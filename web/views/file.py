...
def file(request, project_id):
    ...
    if folder_id.isdecimal():
        parent_object = models.FileRepository.objects.filter(
            id=int(folder_id), file_type=2, project=request.tracker.project
        ).first()
    ...
    if fid.isdecimal():
        edit_object = models.FileRepository.objects.filter(
            id=int(fid), file_type=2, project=request.tracker.project
        ).first()
...
def file_delete(request, project_id):
    ...
    delete_object = models.FileRepository.objects.filter(
        id=fid, project=request.tracker.project
    ).first()
    if delete_object.file_type == 1:
        # 删除文件，将容量还给当前项目的已使用空间
        request.tracker.project.use_space -= delete_object.file_size
    ...
    for folder in folder_list:
        child_list = models.FileRepository.objects.filter(
            project=request.tracker.project, parent=folder
        ).order_by("-file_type")
...
def cos_credential(request, project_id):
    ...
    total_file_limit = (
        request.tracker.price_policy.project_space * 1024 * 1024 * 1024
    )
    ...
    for item in file_list:
        ...
        if item["size"] > per_file_limit:
            msg = "单文件超出限制（最大{}M），文件：{}，请升级套餐。".format(
                request.tracker.price_policy.per_file_size, item["name"]
            )
    ...
    # 总容量进行限制
    # request.tracker.price_policy.project_space  # 项目的允许的空间
    # request.tracker.project.use_space # 项目已使用的空间
    if request.tracker.project.use_space + total_size > total_file_limit:
        return JsonResponse({"status": False, "error": "容量超过限制，请升级套餐。"})

    data_dict = credential(
        request.tracker.project.bucket, request.tracker.project.region
    )
