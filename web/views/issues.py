...
class CheckFilter(object):
    ...
    def __iter__(self):
        for item in self.data_list:
            ...
            if param_url:
                url = "{}?{}".format(
                    self.request.path_info, param_url
            ...
            "filter_list": [
                {
                    "title": "问题类型",
                    "filter": CheckFilter(
                        "issues_type", project_issues_type, request
                    ),
                },
                {
                    "title": "状态",
                    "filter": CheckFilter(
                        "status", models.Issues.status_choices, request
                    ),
                },
                {
                    "title": "优先级",
                    "filter": CheckFilter(
                        "priority", models.Issues.priority_choices, request
                    ),
                },
                {
                    "title": "指派者",
                    "filter": SelectFilter(
                        "assign", project_total_user, request
                    ),
                },
                {
                    "title": "关注者",
                    "filter": SelectFilter(
                        "attention", project_total_user, request
                    ),
                },
            ],
        }
...
def issues_detail(request, project_id, issues_id):
    ...
    issues_object = models.Issues.objects.filter(
        id=issues_id, project_id=project_id
    ).first()
    form = IssuesModelForm(request, instance=issues_object)
    return render(
        request,
        "issues_detail.html",
        {"form": form, "issues_object": issues_object},
    )
...
def issues_change(request, project_id, issues_id):
    ...
    if name in ["subject", "desc", "start_date", "end_date"]:
        ...
        return JsonResponse(
            {"status": True, "data": create_reply_record(change_record)}
        )
            ...
            if name == "assign":
                ...
                if value == str(request.tracer.project.creator_id):
                    ...
                    project_user_object = models.ProjectUser.objects.filter(
                        project_id=project_id, user_id=value
                    ).first()
                ...
                change_record = "{}更新为{}".format(
                    field_object.verbose_name, str(instance)
                )  # value根据文本获取到内容
            else:
                # 条件判断：用户输入的值，是自己的值。
    ...
    if name in ["priority", "status", "mode"]:
        ...
        issues_object.save()
        change_record = "{}更新为{}".format(
            field_object.verbose_name, selected_text
        )
        return JsonResponse(
            {"status": True, "data": create_reply_record(change_record)}
        )
    ...
    if name == "attention":
        ...
        if not value:
            ...
            issues_object.save()
            change_record = "{}更新为{}".format(
                field_object.verbose_name, ",".join(username_list)
            )
...
def invite_join(request, code):
    ...
    exists = models.ProjectUser.objects.filter(
        project=invite_object.project, user=request.tracer.user
    ).exists()
    if exists:
    ...
    # 是否已过期，如果已过期则使用免费额度
    max_transaction = (
        models.Transaction.objects.filter(user=invite_object.project.creator)
        .order_by("-id")
        .first()
    )
    ...
    models.ProjectUser.objects.create(
        user=request.tracer.user, project=invite_object.project
    )
    ...
    return render(
        request, "invite_join.html", {"project": invite_object.project}
    )
